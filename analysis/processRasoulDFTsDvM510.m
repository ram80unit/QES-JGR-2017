...
for m = 1:length(din),
    ...
    HpAmp1(:,m) = dft(m+50).Hp.amp(1:i2,frind);
    tempphase = dft(m+50).Hp.phase(1:i2,frind) * 180/pi;
    HpPhase1(:,m) = tempphase + (360/3e8 * dft(m+50).dftfreqs(frind)*dft(m+50).dist(1:i2)*1000);
...
for m = 21:25,
    amppert(:,m) = 20*log10(HpAmp1(:,m+20)) - 20*log10(HpAmp1(:,m+25));
    phasepert(:,m) = HpPhase1(:,m+15) - HpPhase1(:,m+25);
end
...
% there's an issue with 400 C-km, index #12 here, which is the B30-e3
% simulation. I'm going to manually fix it with some shady dealings.

if fix400,
    m = 12;
    meanAmpPert(m) = 0.5 * (meanAmpPert(m-5) + meanAmpPert(m+5));
    meanPhasePert(m) = 0.5 * (meanPhasePert(m-5) + meanPhasePert(m+5));
    ninetyAmp(m) = 0.5 * (ninetyAmp(m-5) + ninetyAmp(m+5));
    ninetyPhase(m) = 0.5 * (ninetyPhase(m-5) + ninetyPhase(m+5));
    tenAmp(m) = 0.5 * (tenAmp(m-5) + tenAmp(m+5));
    tenPhase(m) = 0.5 * (tenPhase(m-5) + tenPhase(m+5));
end
...
AmpPert.e1.mean = meanAmpPert([1 6 11 16 21]);
AmpPert.e1.ninety = ninetyAmp([1 6 11 16 21]);
AmpPert.e1.ten = tenAmp([1 6 11 16 21]);

AmpPert.e3.mean = meanAmpPert([2 7 12 17 22]);
AmpPert.e3.ninety = ninetyAmp([2 7 12 17 22]);
AmpPert.e3.ten = tenAmp([2 7 12 17 22]);

AmpPert.e5.mean = meanAmpPert([3 8 13 18 23]);
AmpPert.e5.ninety = ninetyAmp([3 8 13 18 23]);
AmpPert.e5.ten = tenAmp([3 8 13 18 23]);

AmpPert.ws1.mean = meanAmpPert([4 9 14 19 24]);
AmpPert.ws1.ninety = ninetyAmp([4 9 14 19 24]);
AmpPert.ws1.ten = tenAmp([4 9 14 19 24]);

AmpPert.ws2.mean = meanAmpPert([5 10 15 20 25]);
AmpPert.ws2.ninety = ninetyAmp([5 10 15 20 25]);
AmpPert.ws2.ten = tenAmp([5 10 15 20 25]);

% phase

PhasePert.e1.mean = meanPhasePert([1 6 11 16 21]);
PhasePert.e1.ninety = ninetyPhase([1 6 11 16 21]);
PhasePert.e1.ten = tenPhase([1 6 11 16 21]);

PhasePert.e3.mean = meanPhasePert([2 7 12 17 22]);
PhasePert.e3.ninety = ninetyPhase([2 7 12 17 22]);
PhasePert.e3.ten = tenPhase([2 7 12 17 22]);

PhasePert.e5.mean = meanPhasePert([3 8 13 18 23]);
PhasePert.e5.ninety = ninetyPhase([3 8 13 18 23]);
PhasePert.e5.ten = tenPhase([3 8 13 18 23]);

PhasePert.ws1.mean = meanPhasePert([4 9 14 19 24]);
PhasePert.ws1.ninety = ninetyPhase([4 9 14 19 24]);
PhasePert.ws1.ten = tenPhase([4 9 14 19 24]);

PhasePert.ws2.mean = meanPhasePert([5 10 15 20 25]);
PhasePert.ws2.ninety = ninetyPhase([5 10 15 20 25]);
PhasePert.ws2.ten = tenPhase([5 10 15 20 25]);
